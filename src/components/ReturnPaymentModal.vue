        ...
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md mx-auto">
            ...
            <div class="p-6">
                <form @submit.prevent="submitForm">
                    <div class="mb-4">
                        <label for="paymentAmount" class="block text-sm font-medium text-gray-700 mb-1">{{ $t('message.return_amount') }}</label>
                        ...
                        <p v-if="errors?.return_amount" class="text-red-500 text-sm mt-1">{{ errors.return_amount[0] }}</p>
                    </div>

                    <div class="mb-4">
                        <label for="paymentMethod" class="block text-sm font-medium text-gray-700 mb-1">{{ $t('message.return_payment_method') }}</label>
                        ...
                        <p v-if="errors?.return_method" class="text-red-500 text-sm mt-1">{{ errors.return_method[0] }}</p>
                    </div>

                    <div class="mb-4">
                        <label for="paymentReference" class="block text-sm font-medium text-gray-700 mb-1">{{ $t('message.return_payment_reference') }}</label>
                        <input v-model="paymentData.return_reference" type="text" id="paymentReference"
                            class="w-full px-4 py-2 border rounded focus:outline-none focus:ring focus:border-blue-300"
                            :class="{ 'border-red-500': errors?.return_reference }" />
                        <p v-if="errors?.return_reference" class="text-red-500 text-sm mt-1">{{ errors.return_reference[0] }}</p>
                    </div>

                    <div class="mb-4">
                        <label for="paymentDate" class="block text-sm font-medium text-gray-700 mb-1">{{ $t('message.return_payment_date') }}</label>
                        ...
                        <p v-if="errors?.return_date" class="text-red-500 text-sm mt-1">{{ errors.return_date[0] }}</p>
                    </div>
