<template>
    <div class="d-flex">
            ...
            <div v-else>
                ...
                <div class="card mb-4 shadow-sm">
                    <div class="card-body">
                        ...
                        <p class="card-text"><strong>Price per Square:</strong> {{
                            formatPrice(contract.price_per_square) }} UZS</p>
                        <p class="card-text"><strong>Contract Amount:</strong> {{ formatPrice(contract.contract_amount)
                            }} UZS</p>
                        <p class="card-text"><strong>Payment Months:</strong> {{ contract.payment_months }}</p>
                        <p class="card-text"><strong>Monthly Payment:</strong> {{ formatPrice(contract.monthly_payment)
                            }} UZS</p>
                    </div>
                </div>
                ...
                <div v-if="contract.downpayment" class="card mb-4 shadow-sm">
                    <div class="card-body">
                        ...
                        <p class="card-text"><strong>Amount:</strong> {{
                            formatPrice(contract.downpayment.payment_amount) }} UZS</p>
                        <p class="card-text"><strong>Method:</strong> {{ contract.downpayment.payment_method }}</p>
                        <p class="card-text"><strong>Reference:</strong> {{ contract.downpayment.payment_reference }}
                        </p>
                        <p class="card-text"><strong>Paid:</strong> {{ contract.downpayment.payment_paid ? 'Yes' : 'No'
                            }}</p>
                    </div>
                </div>
                ...
                <div v-if="contract.lastpayment" class="card mb-4 shadow-sm">
                    <div class="card-body">
                        ...
                        <p class="card-text"><strong>Amount:</strong> {{
                            formatPrice(contract.lastpayment.payment_amount) }} UZS</p>
                        <p class="card-text"><strong>Method:</strong> {{ contract.lastpayment.payment_method }}</p>
                        <p class="card-text"><strong>Reference:</strong> {{ contract.lastpayment.payment_reference }}
                        </p>
                        <p class="card-text"><strong>Paid:</strong> {{ contract.lastpayment.payment_paid ? 'Yes' : 'No'
                            }}</p>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div v-for="record in contract.payment_records" :key="record.id" class="card mb-4 shadow-sm">
                        <div class="card-body">
                            ...
                            <p class="card-text"><strong>Planned Payment:</strong> {{
                                formatPrice(record.payment_amount_plan) }} UZS</p>
                            <p class="card-text"><strong>Paid:</strong> {{ formatPrice(record.payment_amount_paid) }}
                                UZS</p>
                            <p class="card-text"><strong>Customer Debt:</strong> {{ formatPrice(record.customer_debt) }}
                                UZS</p>
                            ...
                            <div v-if="record.payments.length > 0">
                                ...
                                <ul class="list-group">
                                    <li v-for="payment in record.payments" :key="payment.id" class="list-group-item">
                                        ...
                                        <p class="mb-1"><strong>Amount:</strong> {{ formatPrice(payment.payment_amount)
                                            }} UZS</p>
                            ...
                            <!-- Make Payment Button -->
                            <button class="btn btn-primary mt-3" @click="openMakePaymentModal(record.id)">
                                Make Payment
                            </button>
                        </div>
                    </div>
        ...
        <!-- Make Payment Modal -->
        <MakePaymentModal v-if="selectedPaymentRecordId" :paymentRecordId="selectedPaymentRecordId"
...
import MakePaymentModal from '@/components/MakePaymentModal.vue'; // Include the Make Payment Modal
...
// Open the "Make Payment" modal and set the selected payment record ID
const openMakePaymentModal = (paymentRecordId) => {
    selectedPaymentRecordId.value = paymentRecordId;
